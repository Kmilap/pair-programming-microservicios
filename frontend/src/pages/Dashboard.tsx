...
export default function Dashboard() {
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', background: '#07101a', display: 'flex', overflow: 'hidden', fontFamily: 'Inter, system-ui, sans-serif', position: 'relative' }}>
      ...
      {/* ── Sidebar ── */}
      <motion.aside initial={{ x: -60, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.45 }} style={{ width: 220, flexShrink: 0, display: 'flex', flexDirection: 'column', padding: '28px 16px', background: '#081623', borderRight: '1px solid rgba(5,102,141,0.18)', height: '100vh', position: 'relative', zIndex: 10 }}>
        ...
        <nav style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2 }}>
          {navItems.map((item) => (
            <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 10, cursor: 'pointer', fontSize: 13, color: item.active ? '#02C39A' : '#3a6a7a', background: item.active ? 'rgba(2,195,154,0.07)' : 'transparent', borderLeft: item.active ? '2px solid #02C39A' : '2px solid transparent', transition: 'background 0.2s, color 0.2s' }}>
              {item.icon}
        ...
        <div style={{ borderTop: '1px solid rgba(5,102,141,0.18)', paddingTop: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
            <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'rgba(2,195,154,0.1)', border: '1px solid rgba(2,195,154,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 600, color: '#02C39A', flexShrink: 0 }}>
              {user?.name ? (user.name.split(' ')[0][0] + (user.name.split(' ')[2]?.[0] ?? '')).toUpperCase() : 'US'}
            </div>
            <div>
              <div style={{ fontSize: 12, fontWeight: 500, color: '#e8f4f8' }}>
                {user?.name ? user.name.split(' ')[0] + (user.name.split(' ')[2] ? ' ' + user.name.split(' ')[2] : '') : '—'}
              </div>
              <div style={{ fontSize: 10, color: '#3a6a7a' }}>{user?.role ?? '—'}</div>
            </div>
          </div>
          <button
            onClick={async () => { await logout(); navigate('/login', { replace: true }) }}
            style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: '#3a6a7a', background: 'none', border: 'none', cursor: 'pointer', padding: '4px 0' }}
          >
